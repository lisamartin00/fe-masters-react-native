...
const ColorOption = (props) => {
  const { color, onValueChange } = props;

  const [isColorEnabled, setIsColorEnabled] = useState(false);

  const handleValueChange = () => {
    setIsColorEnabled((prevState) => {
      return !prevState;
    }, onValueChange(color, !isColorEnabled));
  };

  return (
    <View style={styles.colorOption}>
      <Text style={styles.label}>{color.colorName}</Text>
      <Switch onValueChange={handleValueChange} value={isColorEnabled} />
    </View>
  );
};
...
const ColorPaletteModal = ({ navigation }) => {
  ...
  const handleValueChange = (color, isColorEnabled) => {
    if (isColorEnabled) {
      setColors((prevState) => {
      ...
      setColors((prevState) => {
        const indexToRemove = prevState.indexOf(color);
  ...
  const handleSubmit = () => {
    if (!paletteName) {
      Alert.alert(
        'Cannot Save Your Color Palette',
    ...
    if (colors.length < MIN_NUM_COLORS) {
      Alert.alert(
        'Cannot Save Your Color Palette',
        `You must choose at least ${MIN_NUM_COLORS} colors`,
        [
          {
            text: 'OK',
          },
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.container}>
        <Text style={styles.label}>
          Name of your color palette: {paletteName}
        </Text>
        <TextInput
          value={paletteName}
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <ColorOption color={item} onValueChange={handleValueChange} />
          )}
        />
